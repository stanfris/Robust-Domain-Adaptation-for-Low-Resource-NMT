...
# Load datasets
medline_dataset = load_dataset('sethjsa/medline_ru_parallel')
scipar_dataset = load_dataset('sethjsa/scipar_en_ru_parallel')

# Select 90k samples from Wikimedia
src_lang = "en"
output_lang = "ru"
selected_train_dataset = medline_dataset['train']
...
def corrupt_text(text):
    ...
    for word in words:
        if len(word) > 3:
            ...
            word = ''.join(random.choice('abcdefghijklmnopqrstuvwxyz') if random.random() < 0.2 else char for char in word)
...
# Save to disk
train_dataset.save_to_disk('../data/corrupted_mixed_medline_scipar')
